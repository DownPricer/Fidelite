...
export function BrandMark({ className }: { className?: string }) {
  return (
    ...
    >
      <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[radial-gradient(circle_at_0%_0%,#b86cff,#4c228c)] text-[var(--ink)] shadow-[0_18px_40px_rgba(0,0,0,0.85)] ring-1 ring-[var(--stroke-strong)]">
        <svg viewBox="0 0 24 24" aria-hidden="true" width={24} height={24} className="h-6 w-6 shrink-0" fill="none">
          <path
      ...
      <span className="text-xl font-semibold tracking-tight">Fideto</span>
    </div>
  );
}

/** Logo compact pour l'app employé (sans texte dupliqué). */
export function EmployeeBrandMark({ className }: { className?: string }) {
  return (
    <div className={cn("employee-brand-mark flex flex-col items-center gap-2 text-center", className)}>
      <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[radial-gradient(circle_at_0%_0%,#b86cff,#4c228c)] text-[var(--ink)] ring-1 ring-[var(--stroke-strong)]">
        <svg viewBox="0 0 24 24" aria-hidden="true" width={20} height={20} className="h-5 w-5 shrink-0" fill="none">
          <path
            d="M4 12.5C4 8.91 6.91 6 10.5 6h3A6.5 6.5 0 0120 12.5c0 3.59-2.91 6.5-6.5 6.5h-3A6.5 6.5 0 014 12.5Z"
            stroke="currentColor"
            strokeWidth="1.4"
