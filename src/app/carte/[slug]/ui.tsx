...
export function LoyaltyCardScreen({ firstName, slug, merchant }: Props) {
  ...
  return (
    <main className="min-h-dvh bg-[var(--page-bg)] pb-20 text-[var(--body-text)]">
      <div className="mx-auto max-w-md px-6 py-8">
        <header className="flex items-center justify-between mb-8">
          ...
          <Link href="/compte" className="group flex items-center gap-2 text-sm font-bold text-[var(--muted-text)] transition-colors hover:text-[var(--panel-text)]">
        ...
        {rewardAvailable ? (
          ...
          <div className="mt-6 flex items-center gap-3 rounded-2xl border border-[var(--border)] bg-[var(--panel-bg)] px-5 py-4 text-[var(--panel-text)] shadow-premium">
            <div className="grid h-8 w-8 place-items-center rounded-full bg-[var(--page-bg)] text-[var(--muted-text)]">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="w-4 h-4">
                <path d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            <p className="text-sm font-bold leading-snug text-[var(--muted-text)]">
              Encore <span className="text-[var(--panel-text)]">{merchant.visitsRequired - points} passages</span> pour débloquer votre cadeau.
            </p>
          </div>
        )}

        {/* Section QR Code */}
        <section className="mt-8 rounded-2xl border border-[var(--border)] bg-[var(--panel-bg)] p-8 text-center text-[var(--panel-text)] shadow-premium">
          {showQr ? (
            <>
              <div className="relative mx-auto h-64 w-64 rounded-xl border border-[var(--border)] bg-[var(--panel-bg)] p-4">
                {qr ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  ...
                  <div className="flex h-full w-full flex-col items-center justify-center gap-3">
                    ...
                    <p className="text-xs font-bold uppercase tracking-wider text-[var(--muted-text)]">Génération...</p>
                  </div>
                )}
              </div>
              <div className="mt-6 space-y-1">
                <div className="flex items-center justify-center gap-2">
                  ...
                  <p className="text-xs font-bold uppercase tracking-widest text-[var(--panel-text)]">QR de votre carte</p>
                </div>
                <p className="text-xs font-medium text-[var(--muted-text)]">
                  Présentez ce code en caisse — il ne change pas.
                </p>
                <p className="text-xs text-[var(--muted-text)]">
                  Dernière mise à jour : <span className="font-semibold text-[var(--panel-text)]">{updatedAt || "–"}</span>
                </p>
              </div>
            ...
            <div className="py-8">
              <p className="text-sm font-medium text-[var(--muted-text)]">Votre QR est masqué pour plus de discrétion.</p>
              <Button className="mt-6 w-full py-4" onClick={() => setShowQr(true)}>
                Afficher mon QR
              </Button>
            </div>
          )}
          {error ? <p className="mt-4 text-xs font-bold uppercase tracking-wider text-[var(--danger)]">{error}</p> : null}
        ...
        <div className="mt-12 text-center sm:hidden">
          <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--muted-text)]">Utilisateur iPhone ?</p>
          <div className="flex items-center justify-center gap-6 text-[var(--muted-text)]">
            <div className="flex flex-col items-center gap-2">
              <div className="grid h-8 w-8 place-items-center rounded-lg bg-[var(--page-bg)]">
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="grid h-8 w-8 place-items-center rounded-lg bg-[var(--page-bg)] text-[var(--panel-text)]">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-4 h-4">
